...
class Admin extends Controller
{
	...
	//用户打印权限专用！解决 查询后打印无权限跳转bug;
	public function userauth6($auth) {
		$comp=explode(',',Session::get('ThinkUser.Competence'));			//当前用户权限获取
		array_pop($comp);
		if (!in_array($auth,$comp)) {
			$this->assign('content', '抱歉，您没有此操作权限');
			exit($this->fetch('public/error2'));
		}
	}
	//用户权限验证3(窗口验证)
	public function win_userauth($auth) {
		...
		array_pop($comp);
		if (!in_array($auth,$comp)) {
			$this->assign('content', '抱歉，您没有此操作权限!请联系管理员添加！');
			exit($this->fetch('public/error'));
		}
	}
	//删除缓存文件
	...
	public function clearcache() {
		...
		if ($request->isAjax()) {
			if (input('request.clear')=='ok') {
				$fileDel = ROOT_PATH.'Runtime/temp';
